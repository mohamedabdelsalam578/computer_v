"""
Evaluate on a split manifest (face branch + full image branch metrics).
...
def main():
    ...
    cfg = ProjectConfig()
    data_cfg = DataConfig()
    output_dir = args.output_dir or str(cfg.results_dir / "ferretnet")
    ...
    # Compute metrics for each branch independently
    print("\n" + "=" * 60)
    print("FACE BRANCH")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("FULL IMAGE BRANCH")
    print("=" * 60)
    full_metrics = compute_metrics(all_labels, full_scores)
    print_metrics(full_metrics)

    # Plot ROC curves
    plot_roc(all_labels, face_scores, full_scores, output_dir)

    # Save results
    save_results(face_metrics, full_metrics, output_dir)
...
def plot_roc(y_true, face_scores, full_scores, output_dir):
    from sklearn.metrics import roc_curve
    plt.figure(figsize=(8, 6))
    for scores, label in [(face_scores, 'Face Branch'), (full_scores, 'Full Branch')]:
    ...
    plt.title('ROC Curves — Face Branch vs Full Image Branch')
    plt.legend()
    plt.tight_layout()
...
def save_results(face_m, full_m, output_dir):
    import json
    results = {
